"""
We want to see whether each retrieval method is getting better.

We use a metric called NDCG@10 to measure how good the search results are.

NDCG@10 looks at the top 10 results and gives a higher score when the relevant documents appear near the top.

1.0 = perfect results
Lower score = worse ranking

We compare four approaches:

BM25 — keyword-based search
Dense retrieval — embedding/vector search
Hybrid (RRF) — combines BM25 + dense search
Hybrid + reranking — combines them and then uses a cross-encoder to improve the ranking

We initially test on 50 randomly selected FiQA test questions to keep the number of reranker API calls small.
"""
...
from utils.reranker import rerank_with_huggingface
...
def reranked_topk(query: str, candidate_k: int = 50, k: int = 10) -> list[str]:
    ...
    return [d for d, _ in rerank_with_huggingface(query, candidates, corpus_by_id, k=k)]
